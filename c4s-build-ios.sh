...
# Source: https://github.com/x2on/OpenSSL-for-iPhone
...
# DEFAULTTARGETS="ios-sim-cross-x86_64"
...
if [ "${BUILD_TYPE}" == "archs" ]; then
  source "${SCRIPTDIR}/c4s.config/ios-scripts/build-loop-archs.sh"
else
  source "${SCRIPTDIR}/c4s.config/ios-scripts/build-loop-targets.sh"
fi
...
mkdir -p ${ROOT_DIR}/c4s.build/libs/iOS
cp ${CURRENTPATH}/lib/libcrypto.a ${ROOT_DIR}/c4s.build/libs/iOS
cp ${CURRENTPATH}/lib/libssl.a ${ROOT_DIR}/c4s.build/libs/iOS
...
# DIOGO - Get includes
echo 'Copy libs'
mkdir -p ${ROOT_DIR}/c4s.build/include
cp -R "${INCLUDE_DIR}" ${ROOT_DIR}/c4s.build/include/
find ${ROOT_DIR} -name '*.h' -print
find ${ROOT_DIR} -name '*.hpp' -print
echo 'Copy done'
