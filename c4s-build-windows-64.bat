...
set LIB_OUTPUT_DIR=%LIB_ROOT_DIR%\c4s.build\libs\%WIN_VERSION%\64\
...
:build
echo "Building %SOURCE% for Windows x64"
cd %SOURCE_DIR%
call perl Configure VC-WIN64A no-comp -no-shared no-asm  --prefix=. && ^
...
xcopy /s/e/y %SOURCE_DIR%\out32\*.lib mkdir %LIB_OUTPUT_DIR%
